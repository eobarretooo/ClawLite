...
- memory diagnostics now also have a first-class live read path through `POST /v1/control/memory/doctor` / `POST /api/memory/doctor`, and the packaged dashboard Knowledge tab can trigger that same safe `memory doctor` snapshot directly through `Run memory doctor` while surfacing the returned doctor snapshot in the Memory card
- memory quality now also has a first-class live control-plane path through `POST /v1/control/memory/quality` / `POST /api/memory/quality`, and the packaged dashboard Knowledge tab can trigger that same persisted `memory quality` snapshot directly through `Run memory quality` while surfacing the returned quality snapshot next to the existing live doctor card
- memory overview now also has a first-class live control-plane path through `POST /v1/control/memory/overview` / `POST /api/memory/overview`, and the packaged dashboard Knowledge tab can trigger that same lightweight `memory overview` snapshot directly through `Run memory overview` while folding the returned coverage/counts snapshot into a more compact memory triage card
- the packaged dashboard Memory card now also promotes guided read-only triage by surfacing the top live memory-quality recommendation and the highest-priority pending proactive-memory suggestion directly next to the existing doctor / overview / quality summaries
- `GET /api/dashboard/state` now also includes additive `memory.remediation` guidance derived from suggestions, quality, overview analysis, and snapshots, and the packaged dashboard Knowledge tab renders that as a compact `Remediation` card so the next safe memory action is visible without reconstructing it from raw counters
- provider probe caching now also derives and persists a compact `capability_summary`, and `clawlite provider status`, `clawlite validate provider`, plus live preflight output reuse that additive `last_capability_probe` view so operators can inspect remote model-list posture without forcing another network call
- the packaged dashboard Tools tab now also surfaces a live `Approval Queue` card backed by `/api/tools/approvals`, so operators can inspect filtered pending approvals and active grants with the dashboard-scoped session flow instead of falling back to raw bearer-token calls or CLI-only queue inspection
- the packaged dashboard Tools tab can now also approve or reject the selected pending tool request directly from that same `Approval Queue` card through the dashboard-scoped `/api/tools/approvals/{request_id}/approve|reject` aliases, reducing another CLI-only approval-review step while keeping grant cleanup separate for now
- the packaged dashboard Tools tab can now also revoke the top visible exact approval grant from that same `Approval Queue` card through the dashboard-scoped `/api/tools/grants/revoke` alias plus `request_id` / `scope`, closing the basic approval/grant cleanup loop without widening the revoke match or opening a separate operator surface
- the packaged dashboard Tools tab now also exposes an explicit exact-grant selector plus compact request `approval_context` hints inside that same `Approval Queue` card, so grant cleanup and approval review no longer depend on an implicit top-grant assumption or raw JSON inspection
...
- that same tools approval audit surface now also carries additive `reason_source` / `reason_summary` per row plus bounded `request_history` when filtered by `request_id`, and the packaged `Approval Audit` card surfaces that latest reason/history directly instead of relying only on raw JSON drill-down
...
- `GET /api/dashboard/state` now also includes additive `provider.health` data derived from cached provider status plus live provider autonomy/telemetry state, and the packaged dashboard Automation tab renders those bounded signals in a compact `Provider Health` card so suppression, cache drift, and capability posture are visible without combining several raw provider blocks manually
- `GET /api/dashboard/state` now also includes additive `provider.budget` data derived from live provider telemetry plus suppression state, and the packaged dashboard Automation tab renders those bounded signals in a compact `Provider Budget` card so operators can distinguish quota exhaustion, rate limiting, and non-budget provider blocks without scraping raw counters
- `GET /api/dashboard/state` now also includes additive `runtime.posture` data derived from autonomy, autonomy-wake, supervisor, and self-evolution state, and the packaged dashboard Automation tab renders those bounded signals in a compact `Runtime Posture` card so approval/no-approval/autonomy posture is visible without opening raw diagnostics
- `GET /api/dashboard/state` now also includes additive `runtime.policy` data derived from self-evolution status/runner policy, and the packaged dashboard Automation tab renders those bounded signals in a compact `Runtime Policy` card so approval mode, canary scope, and manual-only policy blocks are visible without opening raw diagnostics
- `GET /api/dashboard/state` now also includes additive `runtime.policy.drift` data comparing configured self-evolution policy against the effective runtime policy, and the packaged dashboard Automation tab renders it in a compact `Policy drift` card so operators can distinguish an aligned runtime block from a real config/runtime mismatch without opening raw diagnostics
- managed marketplace lifecycle now also exports blocker drill-down on top of the existing live inventory and diagnostics summary: managed skill rows carry `blocker_kind` / `blocker_detail`, visible managed slices include a bounded `blockers` summary, and the packaged dashboard Knowledge tab renders those signals in compact `Managed blockers` cards plus bounded blocked-skill examples
- managed marketplace lifecycle now also exports bounded `blockers.remediation` guidance on top of that blocker drill-down, and the packaged dashboard Knowledge tab renders it in a compact `Managed remediation` card so operators can see the next safe fix path for the dominant visible blocker kind without scanning every row
- tool approval audit now also exposes bounded retention metadata for the current filtered slice, the NDJSON export mirrors that via response headers, and the packaged Tools tab renders the same retention/truncation summary so operators can tell when the current handoff window was clipped by the response limit
- `GET /api/dashboard/state` now also includes additive `channels.posture` data derived from queue pressure plus dispatcher/delivery/inbound/recovery diagnostics, and the packaged dashboard Delivery tab renders that bounded summary in a compact `Channel posture` card so dead-letter backlog, stopped loops, recovery trouble, and healthy steady-state delivery are visible without recombining raw channel-manager counters by hand
- the agent runtime now also exposes an internal `gateway_admin` tool for explicit operator requests to inspect config, apply a bounded config patch plus restart, or restart the gateway outright; that path persists a restart sentinel, blocks background/internal sessions, preserves Telegram topic routing for the post-restart confirmation notice, rejects new admin mutations while a restart is already pending, and keeps the notice on disk when post-boot delivery fails so the confirmation can retry on the next boot instead of disappearing silently
- that same `gateway_admin` path is now schema-aware and fail-closed for writes: `config_schema_lookup` can inspect snake_case config paths before a change, and `config_patch_and_restart` now only accepts a small allowlist of safe tool-tuning fields while rejecting protected config paths such as auth, provider, channel, gateway-auth, `tools.exec.*`, `tools.mcp.*`, `tools.safety.*`, and network-policy/secrets fields
- that same `gateway_admin` path now also supports `config_intent_and_restart`, a bounded preset layer for low-risk tool-tuning changes like default tool timeout, per-tool timeout, workspace-only restriction, and loop-detection tuning, so explicit chat-driven config changes no longer need to rely on raw patch synthesis for those common cases
- that same bounded `gateway_admin` intent layer now also supports `set_web_fetch_limits`, covering safe `tools.web.*` tuning such as timeout, search timeout, redirect limit, max-char budget, and private-address blocking without widening the protected config surface
- that same bounded `gateway_admin` intent layer now also supports a read-only `config_intent_preview` mode, so the agent can resolve a safe preset into exact changed paths, current-vs-next values, and the post-restart confirmation note before writing config or scheduling a restart
- that same bounded `gateway_admin` layer now also supports smaller `tools.web.*` aliases such as `set_web_timeouts`, `set_web_content_budget`, and `set_web_private_address_blocking`, so common low-risk web-fetch asks can map to tighter task-shaped intents instead of always going through the broader fetch-limits preset
- that same `gateway_admin` flow now also supports a read-only `config_patch_preview` path for bounded raw patches, so operators can validate the exact changed paths, current-vs-next values, and restart note before the patch is actually written and restarted
- that same `gateway_admin` flow now also exposes `config_intent_catalog`, a bounded discovery surface for safe config presets that returns summaries, affected path patterns, required/optional arguments, and `requires_any_of` groups before the agent attempts preview or apply
- that same preview/apply admin flow now also returns a strict `preview_token` from `config_intent_preview` and `config_patch_preview`, and the live apply paths can now fail closed when that token no longer matches the patch, note, or current target-config base, preventing a stale preview from being applied after the config drifted in between
- that same bounded `gateway_admin` surface now also supports a low-risk `set_gateway_heartbeat` intent for `gateway.heartbeat.enabled` and `gateway.heartbeat.interval_s`, and heartbeat-interval edits now also keep the legacy `scheduler.heartbeat_interval_seconds` field in sync so older configs cannot silently override the new heartbeat block after restart
