...
- `clawlite validate preflight --docker` now probes Docker/Compose readiness plus the current `clawlite-gateway` container state/health when Docker is available, and `scripts/release_preflight.sh --docker` forwards that same optional deployment check
