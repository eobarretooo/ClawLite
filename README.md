...
- Inbound channel turns now pass a compact allowlisted subset of runtime metadata into the prompt as untrusted context, so the model can see reply/thread/command/media hints without ingesting raw webhook payloads.
