...
```bash
pip install -e ".[all]"
python -m pytest tests/ -q --tb=short   # full suite (2078 passed, 1 skipped)
...
When a user explicitly asks in chat, ClawLite can now also use the internal `gateway_admin` tool to inspect the active config or apply a bounded config patch, schedule a self-restart, and post a confirmation back to the same Telegram chat/topic after the gateway comes back up. That admin path is blocked in background/internal sessions and keeps the restart notice persisted when post-boot delivery fails, so the confirmation is not silently lost on the first transient channel error.
