...
- 🧱 **Autonomy action execution layer** with strict allowlist, denylisted action blocking, per-action cooldown/rate limits, dry-run-only dead-letter replay clamp, additive audit diagnostics, and simulation/explainability controls.
...
- **Working now**
  ...
  - Autonomy action execution layer is active with allowlisted operations (`validate_provider`, `validate_channels`, `diagnostics_snapshot`, `dead_letter_replay_dry_run`), denylisted unknown-action blocking, environment-profile policy layering (`dev`/`staging`/`prod`), runtime policy preset switching with auditable `policy_change` rows (`/v1/control/autonomy/policy`), contextual quality scoring (`base_confidence` - `context_penalty` = `effective_confidence`), per-action cooldown/rate guardrails, quality/degraded runtime gates, additive `autonomy_actions` diagnostics, persisted audit export (`/v1/control/autonomy/audit`), and side-effect-free simulation/explainability controls (`/v1/control/autonomy/simulate`, `/v1/control/autonomy/explain`).
