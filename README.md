...
- `browser.navigate` now prefixes returned page text with a stable “external page content” notice, so browser page reads follow the same untrusted-content discipline as `web_fetch` / `web_search` while staying string-compatible for existing consumers.
