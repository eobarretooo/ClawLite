...
- When deferred turn persistence is enabled, those sync working-memory writes now run off the event loop, and the final file-backed transcript append now does the same while still waiting for durability before the turn returns, so slow post-turn flushes no longer stall unrelated sessions.
