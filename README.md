...
```bash
pip install -e ".[all]"
python -m pytest tests/ -q --tb=short   # full suite (2082 passed, 1 skipped)
...
When a user explicitly asks in chat, ClawLite can now also use the internal `gateway_admin` tool to inspect the active config or apply a bounded config patch, schedule a self-restart, and post a confirmation back to the same Telegram chat/topic after the gateway comes back up. That admin path is now schema-aware as well: it can look up snake_case config paths before editing, and config patches are fail-closed to a small allowlist of safe tool-tuning fields instead of accepting broad arbitrary config mutation. It stays blocked in background/internal sessions and keeps the restart notice persisted when post-boot delivery fails, so the confirmation is not silently lost on the first transient channel error.
