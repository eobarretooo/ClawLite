...
```bash
pip install -e ".[all]"
python -m pytest tests/ -q --tb=short   # full suite (2113 passed, 1 skipped)
...
When a user explicitly asks in chat, ClawLite can now also use the internal `gateway_admin` tool to inspect the active config or apply a bounded config patch, schedule a self-restart, and post a confirmation back to the same Telegram chat/topic after the gateway comes back up. That admin path is now schema-aware as well: it can discover the supported safe presets through `config_intent_catalog`, look up snake_case config paths before editing, dry-run safe presets through `config_intent_preview`, preview bounded raw patches through `config_patch_preview`, and then apply the chosen change through the guarded restart path. Those preview paths now also return a `preview_token` tied to the resolved patch, note, and current target config base, so a later `config_intent_and_restart` or `config_patch_and_restart` can fail closed if the operator is about to apply a stale preview after the config changed underneath it. Raw config patches stay fail-closed to a small allowlist of safe tool-tuning fields, and common low-risk changes can go through `config_intent_and_restart` presets such as gateway-heartbeat tuning, default tool timeout, per-tool timeout, workspace restriction, loop-detection tuning, and bounded `tools.web.*` intents for timeouts, content budget, private-address blocking, or combined fetch-limit updates instead of hand-built patch payloads. Heartbeat interval edits now also keep the legacy `scheduler.heartbeat_interval_seconds` field in sync, so older configs do not silently override the new `gateway.heartbeat.interval_s` value on restart. It stays blocked in background/internal sessions and keeps the restart notice persisted when post-boot delivery fails, so the confirmation is not silently lost on the first transient channel error.
