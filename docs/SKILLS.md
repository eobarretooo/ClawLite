...
That same dashboard card now also exposes the first blocked skills with remediation hints plus dedicated first-class controls for `Refresh skills inventory` and `Doctor blocked skills`, backed by the explicit runtime routes `POST /v1/control/skills/refresh` and `POST /v1/control/skills/doctor`. The refresh path is also available from CLI via `clawlite skills refresh`, while the doctor payload now has a live control-plane equivalent for dashboard/operator use. Those refresh/doctor paths now reuse the same config-aware loader logic too, so explicit `--config` / `--profile` selection stays aligned across CLI and live runtime diagnosis.
