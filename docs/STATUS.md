...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1995 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1995 passed, 1 skipped
...
- Current slice: onboarding/config now keeps `configure` and wizard-backed onboarding aligned with the active `--profile`, persisting and reporting the correct overlay file instead of silently falling back to the base config path
- Next slice: keep pushing Onboarding/Config with another small consistency pass, especially the remaining CLI or dashboard handoff paths that still do not uniformly honor `--profile`
