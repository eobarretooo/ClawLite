...
- Current slice: the packaged dashboard now also mirrors `diagnostics.http.last_error_*` into the local event feed, exposing recent correlated gateway HTTP failures without opening raw diagnostics JSON
- Next slice: add a compact visual summary/card for recent correlated HTTP failures in the dashboard runtime/operator view without jumping straight to full OTEL/tracing
