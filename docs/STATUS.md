...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1984 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1984 passed, 1 skipped
...
- Current slice: Docker deployment now also forwards the common runtime/provider auth envs explicitly through `docker-compose.yml`, with `docker-compose.env.example` documenting the same supported surface for Docker-side overrides
- Next slice: keep pushing Docker/Deployment maturity with a small follow-up such as richer live Docker validation or more guided Docker preflight hints for missing provider/runtime secrets once a daemon is available
