...
The newest skills follow-up now also makes that card actionable: the Knowledge tab surfaces the first blocked skills with remediation hints and adds a dedicated `Refresh skills watcher` button that reuses the supervisor recovery path for `skills_watcher` instead of forcing operators back to the generic supervisor form.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1950 passed, 1 skipped**
...
- The packaged dashboard Knowledge tab now also surfaces the first blocked skills with remediation hints and adds a dedicated `Refresh skills watcher` control that reuses the supervisor recovery path for `skills_watcher`.
...
```bash
python -m pytest tests/ -q --tb=short  # 1950 passed, 1 skipped
...
- Current slice: the packaged dashboard Knowledge tab now also surfaces blocked-skill remediation hints and a dedicated `Refresh skills watcher` control on top of the compact `skills.diagnostics_report()` summary
- Next slice: move from skills remediation visibility into the next small skills-capability increment instead of staying on dashboard-only surfacing
