...
The current OpenClaw parity track is active on `main`. The latest slice brings real Discord interaction streaming into the runtime path, so deferred slash/component replies can now stream by editing the native `@original` interaction response instead of waiting for a second non-streamed channel message.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1892 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1892 passed, 1 skipped
...
- Current slice: Discord runtime parity now includes native interaction streaming on the `@original` response path, on top of the earlier policy/routing, reply-mode, modal, webhook, and voice slices
