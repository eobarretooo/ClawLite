...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2018 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2018 passed, 1 skipped
...
- Current slice: managed marketplace lifecycle is now also exposed as a first-class live control-plane path through `GET /v1/control/skills/managed` / `/api/skills/managed`, and the packaged dashboard Knowledge tab can fetch that full live snapshot on demand through `Inspect managed skills`
- Next slice: keep pushing Skills by turning that live managed inventory into more actionable lifecycle control, such as filtered status views or follow-up managed operations, instead of reopening another passive dashboard-only microfix in a different area
