...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2007 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2007 passed, 1 skipped
...
- Current slice: onboarding/config now also surfaces plausible backends when setup is ambiguous, so multi-provider env/config states no longer fall back to an unassisted provider prompt while the existing detected-credential reuse path stays intact
- Next slice: keep pushing Onboarding/Config by skipping redundant prompts when provider, auth, and base URL are already fully resolved, instead of asking the operator to confirm data the wizard already knows
