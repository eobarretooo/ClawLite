...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1967 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1967 passed, 1 skipped
...
- Current slice: skills remediation now has first-class live control paths through `POST /v1/control/skills/refresh`, `POST /api/skills/refresh`, `POST /v1/control/skills/doctor`, `POST /api/skills/doctor`, `POST /v1/control/skills/validate`, `POST /api/skills/validate`, `clawlite skills refresh`, `clawlite skills validate`, and the packaged dashboard Knowledge tab
- Next slice: move from refresh/doctor/validate controls into the next small skills-capability increment, such as targeted validation filters, managed-skill operations, or deeper doctor actions, instead of staying on dashboard-only surfacing
