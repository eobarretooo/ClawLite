...
- Current slice: the packaged dashboard now turns the live managed inventory path into an actually filterable operator surface, with `status` and `query` controls wired to `GET /v1/control/skills/managed` / `/api/skills/managed` before `Inspect managed skills`
- Next slice: keep pushing Skills by turning that filtered live inventory into more actionable lifecycle control, such as managed sync/remove/update operations or richer status-specific triage, instead of reopening another passive dashboard-only microfix in a different area
