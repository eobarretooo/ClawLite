...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so the direct `send_voice_message(...)` helper now fails closed when Discord's attachment reservation is incomplete and now also clears stale channel error state after a successful native voice-note send, instead of leaving operator health/presence degraded after a recovered upload.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1924 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1924 passed, 1 skipped
...
- Current slice: Discord runtime parity now also makes the direct voice helper fail closed on incomplete attachment-reservation payloads and resets stale voice-send error state after successful recovery, on top of the earlier interaction, webhook, streaming, and voice hardening slices
- Next slice: continue Discord runtime/voice maturity
