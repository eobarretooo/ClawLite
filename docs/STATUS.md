...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1941 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1941 passed, 1 skipped
...
- Current slice: Discord runtime parity now also exposes active reconnect/backoff state alongside the earlier handshake/presence hardening, so operators can see whether the gateway is backing off or actively retrying
