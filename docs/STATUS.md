...
- Current slice: Gateway/API now also exposes baseline HTTP request correlation through `X-Request-ID` headers plus `request_id` in error envelopes, including unhandled `500` responses
- Next slice: extend request correlation into diagnostics/logging or WebSocket surfaces without broad tracing refactors
