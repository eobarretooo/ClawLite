...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1979 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1979 passed, 1 skipped
...
- Current slice: Docker deployment now also exposes image-level `HEALTHCHECK` logic that activates only for `clawlite gateway`, while the compose `clawlite-cli` sidecar explicitly disables that inherited healthcheck so one-off CLI use stays outside the health contract by design
- Next slice: keep pushing Docker/Deployment maturity with a small follow-up such as secrets ergonomics or richer live Docker validation once a daemon is available
