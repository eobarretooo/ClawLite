...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1933 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1932 passed, 1 skipped
...
- Current slice: Discord runtime parity now also surfaces pending startup/reconnect lifecycle state directly in the dashboard and `/discord-status`, on top of the earlier auto-presence refresh, heartbeat, and `READY` / `RESUMED` watchdog slices
