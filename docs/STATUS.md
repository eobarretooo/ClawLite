...
- Current slice: the packaged dashboard runtime tab now also renders a compact `HTTP Correlation` card from `diagnostics.http.last_request_*` and `last_error_*`, surfacing the latest correlated request/failure summary without leaving the operator UI
- Next slice: reuse the correlated HTTP summary in more operator/dashboard surfaces without jumping straight to full OTEL/tracing
