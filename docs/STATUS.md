...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so native `discord_voice` sends now fail closed unless they carry real OGG/Opus audio and a positive duration instead of blindly relabeling arbitrary bytes as `voice-message.ogg`.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1910 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1910 passed, 1 skipped
