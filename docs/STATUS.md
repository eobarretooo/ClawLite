...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity, so `discord_voice` now fails closed for mixed text/embed/component/webhook/modal/poll payloads instead of silently dropping that extra outbound context.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1901 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1901 passed, 1 skipped
