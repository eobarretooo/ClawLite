...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1948 passed, 1 skipped**
...
- Current slice: Gateway/API now also exposes additive WebSocket error correlation through `connect.challenge.params.connection_id` plus `ws.last_connection_*`, `ws.last_request_id`, and `ws.last_error_*` telemetry in diagnostics, building on the earlier HTTP `X-Request-ID` baseline
- Next slice: carry correlation further into dashboard/event/log surfaces without jumping straight to full OTEL/tracing
