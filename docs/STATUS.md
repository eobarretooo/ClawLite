...
The newest memory follow-up now extends that live operator surface again: alongside the existing read-only `memory doctor` route, `POST /v1/control/memory/quality` and `POST /api/memory/quality` now expose the same persisted quality-state report path as `clawlite memory quality`, and the packaged dashboard Knowledge tab can trigger `Run memory quality` while surfacing that live quality snapshot next to the existing doctor view.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2025 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2025 passed, 1 skipped
...
- Current slice: the packaged dashboard now exposes `memory quality` as another live control-plane action, wiring `Run memory quality` to `POST /v1/control/memory/quality` / `/api/memory/quality` and surfacing the resulting quality snapshot directly in the Memory card next to the existing live doctor snapshot
- Next slice: keep pushing `Memory` with one more small operator-safe follow-up, ideally a compact live overview/triage surface that reuses the new doctor + quality snapshots instead of a broader memory refactor
