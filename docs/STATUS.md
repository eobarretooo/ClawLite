...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2009 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2009 passed, 1 skipped
...
- Current slice: onboarding/config now also lets the quickstart happy path skip redundant local-runtime base-URL/model prompts when the provider and runtime defaults are already fully resolved, while the advanced path keeps its explicit override prompts
- Next slice: keep pushing Onboarding/Config by skipping the remaining redundant prompts only in other fully resolved happy paths, instead of asking the operator to reconfirm provider/model/base-url choices the wizard already knows
