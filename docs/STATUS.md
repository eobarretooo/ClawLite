...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so explicit `discord_voice.waveform` values now have to be valid base64 that decodes to Discord's 256-sample shape instead of passing arbitrary strings through to the API.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1915 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1915 passed, 1 skipped
...
- Current slice: Discord runtime parity now also validates explicit `discord_voice.waveform` values before sending native voice notes, on top of the earlier interaction, webhook, streaming, and voice hardening slices
