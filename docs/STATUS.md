...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1954 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1954 passed, 1 skipped
...
- Current slice: skills remediation now has a first-class runtime control path through `POST /v1/control/skills/refresh`, `POST /api/skills/refresh`, `clawlite skills refresh`, and the packaged dashboard Knowledge tab
- Next slice: move from refresh/remediation visibility into the next small skills-capability increment instead of staying on dashboard-only surfacing
