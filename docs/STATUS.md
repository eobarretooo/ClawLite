...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord interaction runtime/webhook parity, so deferred `@original` edits plus follow-up webhook sends now reuse the adapter's normal `429` / `Retry-After` retry discipline instead of behaving like one-shot side paths.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1895 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1895 passed, 1 skipped
