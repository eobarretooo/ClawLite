...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1949 passed, 1 skipped**
...
- Current slice: Gateway/API now also carries `X-Request-ID` into packaged dashboard HTTP/control-plane event entries, so manual chat and operator recovery actions expose the same request handle already returned by the gateway
- Next slice: carry correlation into additional operator/log surfaces without jumping straight to full OTEL/tracing
