...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord runtime health again, so Discord auto-presence now sends through the websocket captured for that cycle, clears stale channel error state after a successful refresh, and the heartbeat loop now closes half-open gateway sockets after a missed ACK so the existing reconnect path can recover transport health automatically.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1928 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1928 passed, 1 skipped
...
- Current slice: Discord runtime parity now also hardens auto-presence refresh recovery and adds a heartbeat-ACK watchdog for half-open gateway sockets, on top of the earlier interaction, webhook, streaming, and voice hardening slices
