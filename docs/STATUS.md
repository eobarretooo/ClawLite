...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1931 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1931 passed, 1 skipped
...
- Current slice: Discord runtime parity now also hardens startup/reconnect health with `READY` / `RESUMED` watchdogs, on top of the earlier auto-presence refresh and heartbeat recovery slices
