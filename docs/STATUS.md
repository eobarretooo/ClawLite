...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so the direct `send_voice_message(...)` helper now treats unrecognized string-style `silent` values fail-closed as non-silent instead of inheriting Python truthiness.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1919 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1919 passed, 1 skipped
...
- Current slice: Discord runtime parity now also makes the direct `send_voice_message(...)` helper fail closed for unrecognized string-style `silent` values, on top of the earlier interaction, webhook, streaming, and voice hardening slices
