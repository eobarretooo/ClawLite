...
The newest memory follow-up now promotes `memory doctor` into that same live operator surface: `POST /v1/control/memory/doctor` and `POST /api/memory/doctor` expose the same read-only diagnostics snapshot as `clawlite memory doctor`, and the packaged dashboard Knowledge tab now offers `Run memory doctor` plus a compact live doctor card instead of leaving that file-integrity view CLI-only.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2023 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2023 passed, 1 skipped
...
- Current slice: the packaged dashboard now exposes `memory doctor` as a live read-only control-plane action, wiring `Run memory doctor` to `POST /v1/control/memory/doctor` / `/api/memory/doctor` and surfacing the resulting doctor snapshot directly in the Memory card
- Next slice: keep pushing `Memory` with one more small operator-safe follow-up, ideally another read-heavy diagnostics/control path rather than a broader memory refactor
