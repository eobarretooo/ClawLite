...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1932 passed, 1 skipped**
...
- Current slice: Discord runtime parity now also exposes pending startup/reconnect lifecycle state in `operator_status()`, on top of the earlier auto-presence refresh, heartbeat, and `READY` / `RESUMED` watchdog slices
