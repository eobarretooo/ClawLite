...
The newest Docker follow-up now also makes `validate preflight` Docker-aware: `clawlite validate preflight --docker` probes `docker compose version`, `docker compose config`, and the current `clawlite-gateway` container state/health when Docker is installed locally, while `scripts/release_preflight.sh --docker` now forwards that same optional deployment check.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1975 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1975 passed, 1 skipped
...
- Current slice: Docker deployment now also has an optional first-class preflight path through `clawlite validate preflight --docker` and `scripts/release_preflight.sh --docker`, surfacing Compose readiness plus current gateway container state/health when Docker is available
- Next slice: keep pushing Docker/Deployment maturity with a small follow-up such as image-level `HEALTHCHECK`, compose/operator diagnostics, or secrets ergonomics instead of reopening dashboard-only work
