...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so the direct `send_voice_message(...)` helper now normalizes string-style `silent` values like `"true"` / `"false"` instead of inheriting Python truthiness outside the metadata path.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1918 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1918 passed, 1 skipped
...
- Current slice: Discord runtime parity now also normalizes string-style `silent` values in the direct `send_voice_message(...)` helper, on top of the earlier interaction, webhook, streaming, and voice hardening slices
