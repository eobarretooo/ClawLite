...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1947 passed, 1 skipped**
...
- Gateway WebSocket `connect.challenge` events now also expose an additive `connection_id`, while diagnostics `ws` telemetry now tracks the latest connection open/close ids, timestamps, path, and last observed request id so operators can correlate live sockets without changing the rest of the WS frame contract.
...
```bash
python -m pytest tests/ -q --tb=short  # 1947 passed, 1 skipped
...
- Current slice: Gateway/API now also exposes additive WebSocket connection correlation through `connect.challenge.params.connection_id` plus `ws.last_connection_*` / `ws.last_request_id` telemetry in diagnostics, building on the earlier HTTP `X-Request-ID` baseline
- Next slice: carry correlation and lightweight lifecycle visibility further into dashboard/logging surfaces without jumping straight to full OTEL/tracing
