...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2014 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2014 passed, 1 skipped
...
- Current slice: skills now export a compact managed-marketplace lifecycle summary in `diagnostics_report()`, and the packaged dashboard Knowledge tab now surfaces tracked/ready/blocked/disabled managed skill state plus a bounded preview without forcing operators back to CLI-only `skills managed`
- Next slice: keep pushing Skills by promoting managed marketplace visibility into a first-class live control path, instead of reopening another passive dashboard-only microfix in a different area
