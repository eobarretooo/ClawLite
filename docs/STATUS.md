...
- Current slice: `Tools` now extend the packaged dashboard with a live `Approval Queue` summary backed by `/api/tools/approvals`, so operators can inspect pending approvals and active grants with dashboard-scoped auth plus compact `status` / `tool` / `rule` filtering instead of dropping back to CLI-only inspection
- Next slice: the next safe high-value follow-up is still inside `Tools`, but now on the action side rather than more read-only summary work — the smallest useful step is likely dashboard/control-plane approval review or grant revoke controls built on top of the new live queue
