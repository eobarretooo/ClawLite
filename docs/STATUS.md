...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord runtime/webhook parity again, so webhook sends now expose Discord's `thread_name` / `applied_tags` forum-thread path and opt into `with_components=true` when components are present.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1905 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1905 passed, 1 skipped
