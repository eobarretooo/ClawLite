...
- Current slice: the packaged dashboard Knowledge tab now also renders a compact `Skills Inventory` summary from `skills.diagnostics_report()`, surfacing availability/runnable coverage, blockers, requirement gaps, contract issues, and watcher health without leaving the operator UI
- Next slice: move from skills visibility into the next small skills-capability increment instead of staying on dashboard-only surfacing
