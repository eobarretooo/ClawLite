...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1922 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1922 passed, 1 skipped
...
- Current slice: Discord runtime parity now also makes the direct voice helper fail closed on incomplete attachment-reservation payloads, on top of the earlier interaction, webhook, streaming, and voice hardening slices
