...
- Current slice: Gateway/API now also keeps HTTP `last_request_*` and `last_error_*` correlation in diagnostics and mirrors failing HTTP requests into gateway logs with `request_id`, method, path, and status
- Next slice: surface the new HTTP correlation summary in additional operator/dashboard views without jumping straight to full OTEL/tracing
