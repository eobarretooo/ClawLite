...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1962 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1962 passed, 1 skipped
...
- Current slice: skills remediation now has first-class live control paths through `POST /v1/control/skills/refresh`, `POST /api/skills/refresh`, `POST /v1/control/skills/doctor`, `POST /api/skills/doctor`, `clawlite skills refresh`, and the packaged dashboard Knowledge tab
- Next slice: move from refresh/doctor controls into the next small skills-capability increment, such as targeted validation or managed-skill operations, instead of staying on dashboard-only surfacing
