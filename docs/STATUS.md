...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so the direct `send_voice_message(...)` helper now strips/sanitizes `channel_id` and `reply_to_message_id` instead of letting blank or whitespace-padded identifiers leak into the Discord voice path.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1921 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1921 passed, 1 skipped
...
- Current slice: Discord runtime parity now also normalizes direct voice helper route identifiers (`channel_id` / `reply_to_message_id`), on top of the earlier interaction, webhook, streaming, and voice hardening slices
