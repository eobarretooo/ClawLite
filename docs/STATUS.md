...
- Current slice: the packaged dashboard overview tab now also renders a compact `Control-Plane Correlation` card from `diagnostics.http.*` and `diagnostics.ws.*`, surfacing the latest correlated request/failure summary without leaving the main operator view
- Next slice: reuse the same correlated gateway summary in more operator/control-plane surfaces without jumping straight to full OTEL/tracing
