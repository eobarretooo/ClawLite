...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2021 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2021 passed, 1 skipped
...
- Current slice: the packaged dashboard now turns managed marketplace lifecycle into a minimally operational control-plane path, adding `Sync managed skills` on top of the existing filtered live inventory route and wiring it to `POST /v1/control/skills/sync` / `/api/skills/sync`
- Next slice: keep pushing Skills, but only one step further: add richer filtered triage or one more small managed-lifecycle control after `sync`, instead of jumping straight to destructive marketplace actions or reopening another passive dashboard-only microfix elsewhere
