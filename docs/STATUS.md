...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord runtime health again, so Discord auto-presence now sends through the websocket captured for that cycle, clears stale channel error state after a successful refresh, the heartbeat loop closes half-open gateway sockets after a missed ACK, the gateway times out stalled `READY` / `RESUMED` handshakes, `operator_status()` exposes that pending lifecycle state directly, both the dashboard and `/discord-status` now surface that wait state without requiring raw JSON inspection, and automatic/manual presence pushes now wait for the current gateway handshake to complete instead of racing a fresh reconnect.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1936 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1936 passed, 1 skipped
...
- Current slice: Discord runtime parity now also waits for the active gateway handshake before auto/manual presence refreshes send `op 3`, on top of the earlier status surfacing, auto-presence refresh, heartbeat, and `READY` / `RESUMED` watchdog slices
- Next slice: continue Discord runtime/health maturity
