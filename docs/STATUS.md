...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord interaction/webhook parity again, so component-bearing interaction follow-ups now also opt into Discord's `with_components=true` webhook contract instead of lagging behind the main webhook transport path.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1911 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1911 passed, 1 skipped
...
- Current slice: Discord runtime parity now keeps component-bearing interaction follow-up webhooks on the same `with_components=true` contract as the main webhook send path, on top of the earlier policy/routing, reply-mode, modal, webhook, streaming, and voice slices
- Next slice: continue Discord runtime/voice maturity or return to deeper tools and skills governance
