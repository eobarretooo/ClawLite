...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1944 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1944 passed, 1 skipped
...
- Current slice: Discord runtime parity now also exposes lifecycle history alongside the earlier handshake/presence/reconnect hardening, so operators can see the last connect/ready/disconnect timestamps and the latest outcome without reading raw logs
