...
The current OpenClaw parity track is active on `main`. The latest slice hardens Discord voice/runtime parity again, so `discord_voice.silent` now parses string-style boolean values like `"true"` / `"false"` correctly instead of inheriting Python's truthiness rules.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1913 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1913 passed, 1 skipped
...
- Current slice: Discord runtime parity now also normalizes string-style `discord_voice.silent` values before sending native voice notes, on top of the earlier interaction, webhook, streaming, and voice hardening slices
