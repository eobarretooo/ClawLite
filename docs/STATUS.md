...
- Full suite: `python -m pytest tests/ -q --tb=short` → **2004 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 2004 passed, 1 skipped
...
- Current slice: onboarding/config now also reuses detected provider credentials during `configure`, so the operator can accept a compatible key already present in config/env instead of retyping it while the provider prompt still starts from the detected backend and preserves the local-runtime reuse path
- Next slice: keep pushing Onboarding/Config with stronger guided setup for ambiguous cases, especially where setup still depends on manual provider choice across multiple plausible backends or should skip redundant prompts entirely
