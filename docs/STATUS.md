...
- Current slice: Gateway/API now also exposes compact WebSocket correlation in `dashboard/state`, letting the packaged dashboard event feed surface recent gateway WS connects/errors using `ws.last_connection_*`, `ws.last_request_id`, and `ws.last_error_*` without scraping full diagnostics
