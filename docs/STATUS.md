...
The current OpenClaw parity track is active on `main`. The latest slice hardens the Discord voice-message path, so attachment reservation and binary voice uploads now reuse the adapter's normal `429` / `Retry-After` retry discipline instead of behaving like a one-shot side path.
...
- Full suite: `python -m pytest tests/ -q --tb=short` → **1893 passed, 1 skipped**
...
```bash
python -m pytest tests/ -q --tb=short  # 1893 passed, 1 skipped
