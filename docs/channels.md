...
- **Voice messages**: pass `metadata["discord_voice"]` through the normal `send()` path to route a channel/DM reply as a native Discord voice note. The payload accepts `audio_bytes`, `audio_base64`, or local `audio_path` / `path`, plus `duration_secs` and optional `waveform` / `silent`.
