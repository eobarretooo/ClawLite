...
- `auto_presence` is now available as a lightweight runtime loop. It maps healthy Discord transport to `online`, degraded transport to `idle`, and unavailable/exhausted transport to `dnd`, with optional custom text overrides. It only pushes `op 3` presence updates when the effective state changes or the minimum update interval allows it, now sends those updates through the websocket captured for that cycle instead of re-reading mutable channel state mid-flight, clears stale channel error state after a successful presence refresh, closes the Discord websocket when a heartbeat ACK is missed so the normal reconnect path can recover a half-open gateway, and now also starts a bounded `READY` / `RESUMED` watchdog after `HELLO` so stalled identify/resume handshakes are forced back through the reconnect path instead of looking healthy forever.
