...
- **Webhooks**: `execute_webhook(...)` now also accepts `thread_id` for thread-targeted delivery, plus `thread_name` and `applied_tags` for forum/media thread creation, reuses the same embed normalization path, and clamps component rows to Discord's five-row limit. When components are present, ClawLite now also opts into Discord's `with_components=true` webhook contract. The normal `send()` path also accepts `metadata["discord_webhook"]` with `id`, `token`, and optional `username` / `avatar_url` / `thread_id` / `thread_name` / `applied_tags` / `wait` to route an outbound message through a Discord webhook instead of the bot token path. That shortcut is intentionally limited to normal text/embed/component delivery today; native reply references and Discord polls still fail closed on the webhook route.
- **Voice messages**: pass `metadata["discord_voice"]` through the normal `send()` path to route a channel/DM reply as a native Discord voice note. The payload accepts `audio_bytes`, `audio_base64`, or local `audio_path` / `path`, plus `duration_secs` and optional `waveform` / `silent`. When that send happens in Discord interaction context, ClawLite now reuses the source `message_id` as a native reply anchor when available, still fails closed for unsupported `discord_ephemeral` or `discord_followup` voice-response requests, rejects mixed text/embed/component/webhook/modal/poll payloads instead of silently dropping that extra outbound context, now also requires real OGG/Opus audio plus a positive duration before it will attempt a native Discord voice upload, validates explicit `waveform` values as base64 that decodes to Discord's 256-sample shape, and normalizes string-ish `silent` values such as `"true"` / `"false"` instead of relying on Python truthiness. The lower-level `send_voice_message(...)` helper now applies that same `silent` normalization directly, treats unrecognized string values fail-closed as non-silent, strips surrounding whitespace from `channel_id` / `reply_to_message_id`, and rejects blank direct `channel_id` values instead of sending malformed voice routes.
- The direct voice helper now also fails closed if Discord's attachment reservation response omits the attachment row or `upload_filename`, and now clears stale channel error state after a successful native voice-note send so operator status/presence do not stay degraded after a recovered upload.
- Automatic waveform generation now also falls back cleanly to the placeholder waveform even if temporary-file setup fails before `ffmpeg` runs, instead of leaking a local-variable error during cleanup.
- The Discord voice-note upload path now reuses the adapter's normal `429` / `Retry-After` retry discipline for attachment reservation and binary upload, instead of treating voice uploads as a one-shot side path.
...
- When inbound traffic arrived via a Discord interaction, normal agent replies now reuse the deferred interaction response path instead of always posting a second channel message. Ephemeral interaction replies now go through the Discord follow-up webhook route so `flags=64` is applied on a valid send path instead of being ignored on a late `@original` edit, the Discord adapter now also caches `application_id` from live interaction payloads so replies still work even if the gateway has not seen `READY` first, built-in operator/approval interactions now ACK ephemerally up front instead of creating a public deferred placeholder first, callers can now opt into a non-ephemeral follow-up message with `metadata["discord_followup"]` when they want a second interaction response instead of editing `@original`, and both deferred `@original` edits plus follow-up webhook sends now reuse the adapter's normal `429` / `Retry-After` retry handling. When those follow-up sends include component rows, ClawLite now also opts into Discord's `with_components=true` webhook flag instead of leaving interaction follow-ups behind the main webhook transport behavior.
- Deferred Discord interaction replies now also stream through the native `@original` response path when the runtime uses `stream_run()`, so slash/component turns can render progressive edits instead of waiting for a second non-streamed channel message. That streaming slice stays fail-closed: explicit `discord_followup` interaction sends still use the normal non-streaming follow-up path.
...
- Inbound Discord interactions now distinguish buttons, string/user/role/channel/mentionable selects, and modal submits. Selects preserve selected values/labels in emitted metadata, while modal submits are routed as fresh inbound turns with compact field text plus `modal_field_ids` / `modal_field_labels`. Outbound `discord_modal` messages register a namespaced trigger button and open the modal with a native Discord interaction callback instead of leaking that trigger click back into the agent loop.
...
- `auto_presence` is now available as a lightweight runtime loop. It maps healthy Discord transport to `online`, degraded transport to `idle`, and unavailable/exhausted transport to `dnd`, with optional custom text overrides. It only pushes `op 3` presence updates when the effective state changes or the minimum update interval allows it, now sends those updates through the websocket captured for that cycle instead of re-reading mutable channel state mid-flight, clears stale channel error state after a successful presence refresh, closes the Discord websocket when a heartbeat ACK is missed so the normal reconnect path can recover a half-open gateway, starts a bounded `READY` / `RESUMED` watchdog after `HELLO` so stalled identify/resume handshakes are forced back through the reconnect path instead of looking healthy forever, and now also suppresses automatic/manual presence pushes until the current gateway handshake has completed so reconnect races cannot emit premature `op 3` updates on a not-yet-ready socket.
...
These are also intercepted before the agent loop. `discord-status` returns the live transport/policy snapshot for the running Discord channel instance, now including additive lifecycle fields like `gateway_session_task_state` and `gateway_session_waiting_for` when the gateway is still waiting for `READY` / `RESUMED`, reconnect/backoff fields like `gateway_reconnect_attempt`, `gateway_reconnect_retry_in_s`, and `gateway_reconnect_state` when the transport is actively recovering, and lifecycle history fields like `gateway_last_connect_at`, `gateway_last_ready_at`, `gateway_last_disconnect_at`, `gateway_last_disconnect_reason`, and `gateway_last_lifecycle_outcome`. The dashboard surfaces that pending handshake/reconnect state directly instead of only leaving it buried in raw JSON, and now also shows a dedicated lifecycle card so operators can tell when the last healthy session started and why the last disconnect happened. `discord-refresh` triggers the same transport restart path exposed by the operator surface, `discord-presence` shows the effective static/auto presence state, and `discord-presence-refresh` forces an immediate `op 3` status update once the current gateway handshake is ready.
