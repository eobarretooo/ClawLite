...
- Inbound Discord interactions now distinguish buttons, string/user/role/channel/mentionable selects, and modal submits. Selects preserve selected values/labels in emitted metadata, while modal submits are routed as fresh inbound turns with compact field text plus `modal_field_ids` / `modal_field_labels`.
