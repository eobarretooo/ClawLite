...
For local runtimes, the onboarding/configure flow now also reuses an already-configured compatible loopback endpoint for the selected provider (`ollama` or `vllm`), normalizes it to the expected `/v1` shape in the prompt, and surfaces that local-runtime hint before the live probe runs. The provider prompt now also starts from a better detected default when the current config, a unique provider override, a single provider-specific env, or a single saved OAuth session already points at one backend. For API-key providers, `configure` now also surfaces a detected compatible key and lets the operator press Enter to reuse it instead of retyping the same credential by hand. When provider, env-backed auth, and base URL are already clearly resolved, `configure` now skips that redundant API-key prompt entirely and just reports the detected credential source before probing; config-backed credentials still stay on the explicit prompt path so the operator can rotate them in-place. When more than one backend looks plausible, the prompt now prints those plausible providers inline instead of dropping straight to an unassisted manual choice or silently auto-switching the default.
...
| `validate preflight --docker` | Also validates Docker/Compose readiness plus the detected runtime stack state/health, and now reports guided provider/gateway secret hints for the Docker launch context; requires `docker` + Compose v2 on the local machine | `clawlite validate preflight --docker` |
...
- `provider status` and `validate provider` now also surface additive `last_live_probe` and `last_capability_probe` blocks when a recent `clawlite validate preflight --provider-live` (or another live provider probe path) has already persisted the latest live result for that provider in local state.
...
| `skills validate` | Refreshes live discovery and returns the current actionable blocked-skills summary through the gateway | `clawlite skills validate --status missing_requirements --query github` |
...
| `skills managed` | Lists managed marketplace skills discovered locally, with optional status/query filters plus aggregate `status_counts` and `blockers` drill-down | `clawlite skills managed --status missing_requirements --query jira` |
...
`skills managed` includes the managed folder `slug`, resolved runtime `status`, and a hint when the skill is blocked or missing requirements, plus global `status_counts` for the full managed inventory. It now also returns an additive `blockers` summary for the currently visible slice, including `count`, `by_kind`, `top_kind`, `top_detail`, `top_hint`, bounded blocker examples, and bounded `blockers.remediation` guidance so operators can see both the dominant managed blocker type and the next safe fix path without manually diffing every row. `skills update` resolves either the slug or the discovered skill name before invoking ClawHub, and successful `install`/`update`/`sync` calls now echo the resolved local marketplace state. `skills search` also includes `local_matches` so an operator can see whether a remote query already exists locally without leaving the CLI. `skills doctor` focuses on broken or blocked skills by default and includes remediation hints for missing env vars, binaries, config keys, invalid contracts, and `skills.allowBundled` policy blocks. `skills config` gives a direct write path for `skills.entries.<skillKey>` in the active base/profile config, so operators can set `apiKey`, merge `env` overrides, or toggle `enabled` without editing raw JSON/YAML. Both `skills doctor` and `skills managed` now accept `--query` for case-insensitive triage by name, skill key, description, requirement, or hint text.
`skills refresh` is the explicit live-control path for runtime discovery. It calls `POST /v1/control/skills/refresh`, returns the refresh outcome plus the current skills/watcher summary, and accepts `--no-force` when you want the runtime debounce policy to decide whether a full refresh is necessary. `skills validate` builds on that same live control plane through `POST /v1/control/skills/validate`: it runs refresh plus the current blocked-skills diagnosis in one step, supports the same `--no-force`, `--all`, `--status`, `--source`, and `--query` filters, and keeps `--config` / `--profile` aligned with the active gateway settings instead of silently falling back to the default config file.
...
| `tools approval-audit` | Lists or exports recent live approval-review and grant-revoke audit rows from the gateway | `clawlite tools approval-audit --format ndjson --action review --request-id req-1 --tool browser --rule browser:evaluate` |
...
`tools approvals`, `tools approval-audit`, `tools approve`, `tools reject`, and `tools revoke-grant` use the live gateway control surface and accept the same `--gateway-url`, `--token`, and `--timeout` flags as `tools catalog`. Approval rows now surface structured context like exec binary/env keys/cwd and browser or web host targets, and `tools approvals` can narrow the queue further with `--tool` and `--rule`. `tools approval-audit` exposes the recent bounded in-memory audit trail for approval reviews and grant revokes, including compact sanitized `approval_context`, `changed`/no-op outcomes, error rows such as actor mismatches, additive summary counts (`action_counts`, `status_counts`, `changed_count`, `error_count`), additive `reason_source` / `reason_summary`, additive `retention` metadata (`limit`, `retained_count`, `matched_count`, `returned_count`, `truncated`), bounded request-scoped `request_history` when `--request-id` is supplied, and `--format ndjson` when operators want a bounded export instead of the normal JSON envelope. The NDJSON export now also preserves the same retention summary via response headers for programmatic callers. When a gateway token is configured, the approval/grant endpoints require that token even on loopback. When a request row includes `requester_actor`, that review is actor-bound: inspect it from CLI if you want, but approve/reject it from the original Telegram/Discord interaction instead of the generic CLI command. Generic CLI-triggered reviews are recorded as `control-plane`, not as the optional `--actor` label.
