...
- `config_patch_preview` provides the same read-only preview shape for a bounded raw patch when the change does not map cleanly to one of the predefined safe intents yet.
