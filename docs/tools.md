...
The packaged dashboard now reuses that same live queue through `/api/tools/approvals`. The Tools tab ships an `Approval Queue` card with `status` / `tool` / `rule` filters, shows the first matching request plus grant scope, auto-selects the top pending `request_id` when possible, and can approve or reject that selected request directly from the packaged shell through the dashboard-scoped session flow instead of requiring the raw gateway bearer token to stay in the browser tab.
