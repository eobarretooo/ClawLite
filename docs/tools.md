...
- `gateway_admin`: `action`, `path`, `intent`, `patch`, `note`, `preview_token`, `restart_delay_s`, `enabled`, `tool_name`, `timeout_s`, `search_timeout_s`, `history_size`, `repeat_threshold`, `critical_threshold`, `max_redirects`, `max_chars`, `block_private_addresses`
