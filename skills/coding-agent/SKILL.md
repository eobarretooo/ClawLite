...
metadata:
  {
    "clawdbot": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in ClawLite's own project folder!**
...
8. **NEVER start Codex in ~/.clawlite/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/clawlite/** - that's the LIVE ClawLite instance!
...
For long-running background tasks, append a wake trigger to your prompt so ClawLite gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
clawlite system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: clawlite system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
