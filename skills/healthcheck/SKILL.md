---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ClawLite deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ClawLite cron scheduling for periodic checks, or version status checks on a machine running ClawLite (laptop, workstation, Pi, VPS).
---

# ClawLite Host Hardening

## Overview

Assess and harden the host running ClawLite, then align it to a user-defined risk tolerance without breaking access. Use ClawLite security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ClawLite changes the host firewall, SSH, or OS updates; it does not.
...
5. ClawLite gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ClawLite can access sensitive data.
...
### 2) Run ClawLite security audits (read-only)

As part of the default read-only checks, run `clawlite security audit --deep`. Only offer alternatives if the user requests them:

1. `clawlite security audit` (faster, non-probing)
2. `clawlite security audit --json` (structured output)

Offer to apply ClawLite safe defaults (numbered):

1. `clawlite security audit --fix`

Be explicit that `--fix` only tightens ClawLite defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ClawLite version/update status (read-only)

As part of the default read-only checks, run `clawlite update status`.
...
- Credential hygiene notes (location of ClawLite creds, prefer disk encryption)
...
- ClawLite security audit (re-run)
...
After ClawLite install or first hardening pass, run at least one baseline audit and version check:

- `clawlite security audit`
- `clawlite security audit --deep`
- `clawlite update status`

Ongoing monitoring is recommended. Use the ClawLite cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `clawlite cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `clawlite update status`
...
Before creating, `clawlite cron list` and match on exact `name`. If found, `clawlite cron edit <id> ...`.
If not found, `clawlite cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `clawlite update status` (preferred for source checkouts and channels)
2. `npm view clawlite version` (published npm version)

## ClawLite command accuracy

Use only supported commands and flags:

- `clawlite security audit [--deep] [--fix] [--json]`
- `clawlite status` / `clawlite status --deep`
- `clawlite health --json`
- `clawlite update status`
- `clawlite cron add|list|runs|run`

Do not invent CLI flags or imply ClawLite enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ClawLite compaction:
