...
class RemoteProviderStreamingTests(unittest.TestCase):
    ...
    def test_codex_stream_429_retries_then_succeeds(self) -> None:
        stream_429_a = MagicMock()
        stream_429_a.raise_for_status.side_effect = self._http_status_error(429)
        ...
        stream_ok.raise_for_status.return_value = None
        stream_ok.iter_lines.return_value = [
            'data: {"choices":[{"delta":{"content":"co"}}]}',
            'data: {"choices":[{"delta":{"content":"dex"}}]}',
            "data: [DONE]",
        ]
        ...
        with patch("clawlite.runtime.streaming.httpx.Client", return_value=cm_client):
            with patch("clawlite.runtime.streaming.time.sleep") as sleep_mock:
                out = "".join(run_remote_provider_stream("ping", "openai-codex/gpt-5.3-codex", "sk-codex-token"))
    ...
    def test_codex_stream_429_exhausted_returns_clear_message(self) -> None:
        stream_429_a = MagicMock()
        stream_429_a.raise_for_status.side_effect = self._http_status_error(429)
        ...
        with patch("clawlite.runtime.streaming.httpx.Client", return_value=cm_client):
            with patch("clawlite.runtime.streaming.time.sleep") as sleep_mock:
                with self.assertRaises(ProviderExecutionError) as ctx:
                    "".join(run_remote_provider_stream("ping", "openai-codex/gpt-5.3-codex", "sk-codex-token"))
    ...
    def test_codex_oauth_stream_429_retries_then_succeeds(self) -> None:
        with patch("clawlite.runtime.streaming.resolve_codex_account_id", return_value="acc_123"):
            with patch(
                "clawlite.runtime.streaming.run_codex_oauth_stream",
                side_effect=[
